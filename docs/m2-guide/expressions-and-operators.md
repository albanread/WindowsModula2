...
`INCL(s, e)` and `EXCL(s, e)` are pervasive procedures that add and remove a single element; they are more efficient than `s := s + SetType{e}` for single-element updates. See [The standard environment](standard-environment.md).
...
These are pervasive identifiers — no import needed. They are described in full in [The standard environment](standard-environment.md).

---
[NewM2 Guide home](index.md) · [Declarations & types](declarations-and-types.md) · [Statements & control flow](statements-and-control-flow.md)
