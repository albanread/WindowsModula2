...
BEGIN
  ...
  len:=LENGTH(a);
  (* ISO 10514-1: width=0 means "as many positions as needed" — no
     leading space. The XDS source wrote a single space at width=0
     for readability but that conflicts with the spec literally and
     with NewM2's existing runtime-stub `nm2_io_write_int` behaviour. *)
