...
(* Ported to NewM2 2026-05-14 from XDS 2.60 lib/src/isoimp. Apache-2.0.
   Integration notes:
     - Concretizes the opaque IOChan.ChanId as IOLink.DeviceTablePtr,
       matching the XDS source. Every IOChan operation is a one-liner
       that dispatches through the device table's procedure-pointer
       fields installed by the device module (TermFile, StreamFile, …).
     - InvalidChan() returns a module-private `inv` value initialised in
       the BEGIN block to a freshly-allocated device table whose fields
       still point at IOLink's "raise notAvailable" stubs — calling any
       operation on the invalid channel therefore raises notAvailable.
*)
