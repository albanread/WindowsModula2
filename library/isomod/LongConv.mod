...
(* Ported to NewM2 2026-05-13 from XDS 2.60 lib/src/isoimp. Apache-2.0.
   Integration notes:
     - Structural twin of RealConv.mod; only the float type (LONGREAL)
       and the imported string-conversion module (LongStr) differ.
     - Flattened `<* IF EXCEPTIONS *> %ELSE XRaise %END *>` chains;
       XRaise removed.
     - Dropped `<*+ m2extensions *>` pragma.
     - **Upstream bug fixed**: XDS LongConv.mod's `raise` raised the
       string "RealConv.Exception" (the mirror of the RealConv bug —
       both modules carry each other's exception text). Corrected to
       "LongConv.Exception".
*)
