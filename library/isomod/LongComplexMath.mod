...
(* Ported to NewM2 2026-05-13 from XDS 2.60 lib/src/isoimp. Apache-2.0.
   Integration notes (see ComplexMath.mod for the canonical rationale):
     - Body is structurally identical to ComplexMath.mod; only the
       type aliases (`complex`, `real`) and the imported math module
       differ. `IMPORT math := LongMath` (LONGREAL transcendentals).
     - Dropped `<*+ M2EXTENSIONS *>` / `<*- M2EXTENSIONS *>` pragmas.
     - Replaced one use of XDS string-concat operator `+` in
       `arg(z = 0)` with `Strings.Concat`.
     - The .def file's `CMPLX(0.0, 1.0)` etc. relies on the front end
       resolving the CMPLX result type to LONGCOMPLEX from the
       declaration context. If sema needs an explicit form
       (`LONGCMPLX(...)` or context-typed literals), this is an
       integration point.
*)
...
BEGIN
  ...
  ELSIF re < 0. THEN
    ...
    END;
  ELSIF im = 0. THEN
    Strings.Concat(name, "arg: zero argument", msg);
    EXCEPTIONS.RAISE(source, 0, msg);
  ELSIF im > 0. THEN
    RETURN  pi / 2.;
  ELSE
    RETURN -pi / 2.;
