...
(* Ported to NewM2 2026-05-13 from XDS 2.60 lib/src/isoimp. Apache-2.0.
   Integration notes:
     ...
     - Replaced one use of XDS string-concat operator `+` in
       `arg(z = 0)` with `Strings.Concat` plus a local buffer. The
       message text is identical.
     - Imports `math := RealMath` — that's the already-ported module,
       not an XDS internal. No `xMath` substitution needed here.
*)
