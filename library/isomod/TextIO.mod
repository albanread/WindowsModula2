...
(* Ported to NewM2 2026-05-14 from XDS 2.60 lib/src/isoimp. Apache-2.0.
   Integration notes:
     - Dropped XDS `<*+ M2EXTENSIONS *>` pragma — not relevant outside XDS.
     - Dropped the constant-parameter modifier (`s-: ARRAY OF CHAR`
       → plain `s: ARRAY OF CHAR`); NewM2 sema doesn't yet implement
       that extension and these callsites are read-only anyway.
*)
...
BEGIN
  ...
  (* ASSUME : IF res=ok THEN ch = the first char of token *)
